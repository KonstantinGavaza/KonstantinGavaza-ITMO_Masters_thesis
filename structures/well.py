...
def check_correctness_well(well, field):
    ...
    for i in range(len(field.impossible_points[0])):
        if abs(field.impossible_points[0][i] - well.start[0]) + abs(field.impossible_points[1][i] - well.start[1]) < 0.5:
            return False
...
class Well():
    def __init__(self, position_start, position_end, r):
        ...
        well_cells:
        '''
        self.start = (int(position_start[0]), int(position_start[1]))
        self.end = (int(position_end[0]), int(position_end[1]), int(position_end[2]))
        self.r = r
        
        self.start_end = [self.start[0], self.start[1], self.end[0], self.end[1], self.end[2]]
        
        self.len_wells_m = (((20 * (self.start[0] - self.end[0])) ** 2 + (10 * (self.start[1] - self.end[1])) ** 2 + (2 * self.end[2]) ** 2) / 3.28) ** 0.5
