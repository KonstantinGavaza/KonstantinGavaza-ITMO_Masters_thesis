...
def get_random_structure(domain, invalid_points) -> Structure:
    ...
    for _ in range(num_pols):
        polygon = get_random_poly(parent_structure=structure,
                                  domain=domain, invalid_points = invalid_points)
...
def get_random_poly(parent_structure: Optional[Structure],
                    domain: Domain, invalid_points) -> Optional[Polygon]:
    ...
    try:
        ...
        if occupied_area is None:
            ...
            polygon = create_poly(centroid,
                                  ...
                                  domain,
                                  geometry)
            while not check_poly(polygon, geometry, invalid_points):
                polygon = create_poly(centroid,
                                      sigma,
                                      domain,
                                      geometry)
...
def check_poly(poly, geometry: 'Geometry', 
               invalid_points, soft_ = True):
    if soft_:
        return True
    ...
    poly = Polygon('dist_help', points=points)
    
    structure = Structure(polygons=[poly])
    
    for i in range(len(invalid_points[0])):
        point = Point(invalid_points[0][i], invalid_points[1][i])
        dist = distance(point, structure, geometry)
        if dist < 0.5:
            return False
    return True

def check_structure(structure, geometry: 'Geometry', 
                    invalid_points, soft_ = True):
    if soft_:
        return True
    poly = structure.polygons[0]
