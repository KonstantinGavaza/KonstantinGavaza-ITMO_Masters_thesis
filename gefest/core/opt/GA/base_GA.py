...
from gefest.core.structure.structure import check_structure


class BaseGA:
    def __init__(self, params, calculate_objectives,
                 evolutionary_operators, task_setup: Setup, init_pop=None, 
                 visualiser=None, invalid_points = [[],[]], coord_wells = [[], []]):
        """
    ...
    def __init_populations(self):
        ...
        gens = self.init_population(self.params.pop_size, self.task_setup.domain, invalid_points = self.invalid_points)
    ...
    def fitness(self):
        self.calculate_objectives(population=self._pop, coord_wells = self.coord_wells, invalid_points = self.invalid_points)
    ...
    def reproduce(self, selected):
        ...
        for pair_index in range(0, len(selected) - 1):
            ...
            while not check_structure(child_gen, self.task_setup.domain.geometry, self.invalid_points):
                child_gen = self.crossover(s1=p1.genotype, s2=p2.genotype,
                                          domain=self.task_setup.domain,
                                          rate=self.params.crossover_rate)
