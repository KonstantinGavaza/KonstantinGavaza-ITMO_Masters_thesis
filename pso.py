...
class PSO():
    def __init__(self, field, n_particles, parameters_for_well, c0=1.193, c1=1.193, w=0.721, npv = None, pop = None, best = None):
        self.field = field
    ...
    def well_operation_coord_w_v(self, well, vector):
        ...
        for i in range(len(well)):
            new_well = Well((well[i].start[0] + vector[5 * i + 0], well[i].start[1] + vector[5 * i + 1]), (well[i].end[0] + vector[5 * i + 2], 
    ...
    def well_operation_coord_diff_w(self, well_1, well_2, coeff):
        ...
        for i, well_1_i in enumerate(well_1):
            res += [(well_1_i.start[0] - well_2[i].start[0]) * coeff, (well_1_i.start[1] - well_2[i].start[1]) * coeff, (well_1_i.end[0] - well_2[i].end[0]) * coeff, 
    ...
    def update_velocity(self, x, v, p_best, g_best):
        ...
        new_v = self.w*v + self.well_operation_coord_diff_w(p_best, x, self.c0 * r) + self.well_operation_coord_diff_w(g_best, x, self.c1 * r)
        return new_v
